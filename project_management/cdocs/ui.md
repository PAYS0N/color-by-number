...
**HistoryScreen (My Work tab)**: Combined grid of saved puzzles and pixel art (newest first), each showing thumbnail preview. Puzzle cards show a completion badge or progress percentage. Pixel art cards show a brush badge. Tapping a puzzle opens InProgressPuzzleDialog (Resume/Delete) or CompletedPuzzleDialog (replay/download/delete). CompletedPuzzleDialog toolbar has: Delete button (left), then on the right: a Code icon button that exports dense-format JSON to Downloads (filename `puzzle_<id>_<timestamp>.json`), a Download icon button that saves the completed image as PNG to Pictures, and a Close button. Tapping pixel art opens PixelArtDialog (Resume/Delete with confirmation, plus a Code icon button that exports sparse JSON to the Downloads folder). Both InProgressPuzzleDialog and PixelArtDialog have an X close button in the title row (Icons.Default.Close). Both dialogs match Material 3 style. No back arrow; navigation via bottom tabs.
